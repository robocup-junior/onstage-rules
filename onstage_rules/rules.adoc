= RoboCupJunior OnStage Draft Rules 2026
{docdate}
:toc: left
:sectanchors:
:sectlinks:
...
All teams are judged in the following areas: Technical Description Poster, Technical Demonstration Video, Technical Interview, and the OnStage Performance. Teams must highlight four of their robot(s) *features* in the work they present to the judges. Ask the following question - “What do we believe are our best system/sensor integration, electromechanical design, interaction, or software solutions implemented on our robot(s)? Why/how does our project stand out from other teams’ projects?” The aim should be to present how the chosen features are integrated to enhance performance quality in unique and innovative ways. A good feature is one that is specific, clearly defined, and demonstrates innovation, technical complexity, and integration into the performance. A good rule of thumb is that a feature is defined with a short sentence rather than a word or concept.

{++Examples of good features:++}
[compact]
- Hand gesture recognition with pretrained dataset for sign-language interpretation and feedback through robot1 movement
- Robotic arm capable of grabbing door noob (prop) and opening the door and preassure sentitive to not breaking fragile objects
- PID algorithm for adjusting the car robot's deviation from a team member using ultrasonic sensors as feedback for error
