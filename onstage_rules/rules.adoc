...
All teams are judged in the following areas: Technical Description Poster, Technical Demonstration Video, Technical Interview, and the OnStage Performance. 
...
{~~All movements or interactions that happen outside the performance area~>Any part of the robots and the interactive props that are outside the performance area~~} will not be considered for the scoring, but will not lead to deductions.
...
{++Teams must be prepared to conduct a live demonstration proving that all four selected features are fully working.++}
...
Robots should be presented without their costumes and key {++four++} features of the technologies used should be visible to the audience.
...
Teams must read the Score Sheets carefully so that their robot performance{++, technical video, poster and interview++} cover as much judging criteria as possible,

[[totaling]]
=== Totaling

The total score of each team is calculated by {~~combining the scores~>combining the weighted scores as described in the scoresheet document~~} from the team’s Technical Interview, the Technical Demonstration, and the OnStage Performance.
...
- Supplemental Document: How to create a good technical demonstration video and description poster, What is innovative?
- Video: How to create valuable human robot interactions
- Explanation: How to select a good feature
- Glossary: stage set, scenery, props, music, sound effects, etc.
